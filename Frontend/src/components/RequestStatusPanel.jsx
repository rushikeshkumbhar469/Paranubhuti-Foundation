...
/**
 * @param {string} initialRequestId - Pre-fill the tracking input
 * @param {string} docType - 'id' | 'receipt' | 'certificate' — restricts look-ups to this type only
 */
export default function RequestStatusPanel({ initialRequestId = '', docType = '' }) {
  ...
  // Validate that the result matches the expected docType for this panel
  const validateDocType = (data) => {
    if (!docType) return true; // no restriction
    return data.docType === docType;
  };
  ...
  const performLookup = async (id) => {
    ...
    try {
      ...
      if (!validateDocType(result.data)) {
        const expectedLabel = docTypeLabels[docType] || docType;
        showToast('error', `This tracking ID belongs to a different document type. Please check it on the correct page.`);
        setError(`This tracking ID is not a ${expectedLabel} request. Use the correct section to check its status.`);
        return;
      }
  ...
  useEffect(() => {
    if (initialRequestId) {
      ...
      performLookup(initialRequestId);
    }
  }, [initialRequestId]);
  ...
  const handleCheck = async (e) => {
    ...
    await performLookup(requestId.trim());
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200">
      ...
      {toast && (
        <div
          className={`mb-4 flex items-center justify-between gap-3 rounded-xl px-4 py-3 text-sm font-medium shadow-lg transition-all animate-fade-in ${toastStyles[toast.type]}`}
      ...
      <form onSubmit={handleCheck} className="mt-4 flex flex-col gap-3 sm:flex-row">
        <input
          ...
          placeholder="e.g. PFHW-6485-6568"
